...
router.post('/create', async (req, res) => {
  try {
    const parsed = completeCreditCardSchema.safeParse(req.body);
    ...
    // Insert credit card specific details
    await db.query(
      `INSERT INTO credit_card_applications (
        application_id, card_type, credit_limit_requested, card_delivery_address,
        supplementary_cards, reward_program, auto_debit_setup
      ) VALUES ($1, $2, $3, $4, $5, $6, $7)`,
      [
        applicationId, data.creditCardDetails.cardType, data.creditCardDetails.creditLimitRequested,
        data.creditCardDetails.cardDeliveryAddress, data.creditCardDetails.supplementaryCards,
        data.creditCardDetails.rewardProgram, data.creditCardDetails.autoDebitSetup
      ]
    );

    res.status(201).json({
      success: true,
      message: 'Credit card application created successfully',
      applicationId: applicationId,
      application: application
    });

  } catch (error) {
    console.error('Error creating credit card application:', error);
    res.status(500).json({ 
      error: 'Failed to create credit card application', 
...
router.get('/:applicationId', async (req, res) => {
  try {
    ...
    const [personalDetails, addressDetails, employmentDetails, references, 
           bankingDetails, creditCardDetails] = await Promise.all([
      ...
      db.query('SELECT * FROM credit_card_applications WHERE application_id = $1', [applicationId])
    ]);
    ...
    const completeApplication = {
      ...
      creditCardDetails: creditCardDetails.rows[0] || null
    };
    ...
    res.status(500).json({ 
      error: 'Failed to fetch credit card application', 
      details: error.message 
    });
  }
});
...
router.get('/', async (req, res) => {
  try {
    ...
    let query = `
      SELECT a.*, pd.first_name, pd.last_name, cc.card_type, cc.credit_limit_requested
      FROM applications a
      LEFT JOIN personal_details pd ON a.application_id = pd.application_id
      LEFT JOIN credit_card_applications cc ON a.application_id = cc.application_id
      WHERE a.product_type = 'creditcard'
    `;
    ...
    // Get total count
    let countQuery = 'SELECT COUNT(*) FROM applications WHERE product_type = \'creditcard\'';
    const countParams = [];
    ...
    res.status(500).json({ 
      error: 'Failed to fetch credit card applications', 
      details: error.message 
    });
  }
});
